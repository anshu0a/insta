...
export default function views({ setResponseMessage, opt, setOpt, userid, storyid }) {
    ...
    return (
        ...
        <div className="inopton">
            <h3> <img onClick={() => setOpt((pre) => ({ ...pre, page: 0 }))} src="/svg/bref.svg" /> <span>Views</span></h3>
            {
                alldata.length > 0 ?
                    alldata.map((one) => (
                        <div key={one.user._id} >
                            <img  onClick={()=> navigate("/profile/"+one.user.username)} src={one.user.pic.path}></img>
                            <a href={"/profile/"+one.user.username}>{one.user.username}</a>
                        </div>
                    )) : <p>No views on this story</p>
            }
